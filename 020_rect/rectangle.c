...
rectangle_t intersection(rectangle_t r1, rectangle_t r2) {
  ...
  r2 = canonicalize(r2);
  rectangle_t new;
  //  printf("r1.x=%d,r2.x=%d", r1.x, r2.x);
  new.x = max(r1.x, r2.x);
  new.y = max(r1.y, r2.y);
  new.width = min(r1.x + r1.width, r2.x + r2.width) - new.x;
  if (new.width < 0) {
    new.width = 0;
    new.height = 0;
    return new;
  }
  new.height = min(r1.y + r1.height, r2.y + r2.height) - new.y;
  if (new.height < 0) {
    new.width = 0;
    new.height = 0;
  }
  //  printf("\n%d,%d,%d,%d", new.x, new.y, new.x + new.width, new.y + new.height);
  return new;
}
