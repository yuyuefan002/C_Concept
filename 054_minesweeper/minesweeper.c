...
board_t * makeBoard(int w, int h, int numMines) {
  ...
  new_board->board = NULL;
  for (int i = 0; i < h; i++) {
    int * lines = malloc(new_board->width * sizeof(*lines));
    new_board->board = realloc(new_board->board, (i + 1) * sizeof(*new_board->board));
    for (int j = 0; j < w; j++) {
      lines[j] = UNKNOWN;
    }
    new_board->board[i] = lines;
    lines = NULL;
  }
  /*new_board->board = malloc(new_board->height * sizeof(*new_board->board));
  for (int j = 0; j < new_board->height; j++) {
    new_board->board[j] = malloc(new_board->width * sizeof(**new_board->board));
  }
...
int countMines(board_t * b, int x, int y) {
  //WRITE ME!
  int count = 0;
  // int w_low = (x - 1 >= 0) ? x - 1 : 0;
  //int w_upper = (x + 1 < b->width) ? x + 1 : b->width - 1;
  //int h_low = (y - 1 >= 0) ? y - 1 : 0;
  //int h_upper = (y + 1 < b->height) ? y + 1 : b->height - 1;
...
void freeBoard(board_t * b) {
  //WRITE ME!
  //totally 3 block to free
  //the first is the block storing the width data;
  //the second is the block storing the height data;
  // the third is the block storing the struct board_t
  for (int i = 0; i < b->height; i++) {
    free(b->board[i]);
  }
  free(b->board);
